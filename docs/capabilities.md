...
- **Deterministic creative pipeline:** build goals ("redesign the YouTube homepage", "full-stack ecommerce site") that outlive planner failures still ship a real **multi-file project folder** under `projects/<name>/` — `index.html` + `css/styles.css` + `js/app.js` + `README.md`, plus a dependency-free stdlib `server.py` when the goal implies backend work — generated via `execute_code` with zero further LLM calls, immune to rate limits
